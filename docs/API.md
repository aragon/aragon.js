...
Listens for events, passes them through `reducer`, caches the resulting state, and re-emits that state for easy chaining. **Should be** used as the main "event loop" in application background scripts running in a WebWorker.
