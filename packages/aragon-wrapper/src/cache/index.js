...
export default class Cache {
  ...
  async set(key, value) {
    ...
    await Promise.resolve(this.db.set(this.getCacheKeyPath(key), value).write())

    this.changes.next({ key, value })
  }

  get(key, defaultValue) {
    return this.db.get(this.getCacheKeyPath(key)).value() || defaultValue
  }

  update(key, transition) {
    return this.set(key, transition(this.get(key)))
  }
   ...
   * @return {Observable} Value of a key in cache over time
   */
