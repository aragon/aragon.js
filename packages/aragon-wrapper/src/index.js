...
export default class Aragon {
  ...
  async initAcl({ aclAddress } = {}) {
    ...
    // Set up ACL proxy
    this.aclProxy = makeProxy(
      aclAddress,
      "ACL",
      this.web3,
      this.kernelProxy.initializationBlock
    );
    ...
    const currentEvents$ = this.aclProxy
      .events(null, { fromBlock: cacheBlockHeight + 1 })
      .pipe(
        startWith({
        ...
        ([permissions], event) => {
          ...
          if (eventData.app) {
            ...
            if (event.event === SET_PERMISSION_EVENT) {
              ...
              const allowedEntitiesSet = new Set(
                dotprop.get(appPermissions, key, [])
              );
    ...
    const cachedPermissions$ = cachedPermissions
      ? of(makeAddressMapProxy(cachedPermissions))
      : of();
    this.permissions = concat(cachedPermissions$, fetchedPermissions$).pipe(
      publishReplay(1)
    );
  ...
  initApps() {
    ...
    const applicationInfoCache = new AsyncRequestCache(cacheKey => {
      ...
      return (
        getAragonOsInternalAppInfo(appId) ||
        getApmAppInfo(appId) ||
    ...
    const installedApps$ = this.permissions.pipe(
      ...
      distinctUntilChanged((oldProxies, newProxies) => {
        ...
        const intersection = new Set(
          newProxies.filter(newProxy => oldSet.has(newProxy))
        );
        return intersection.size === oldSet.size;
      }),
      // Add Kernel as the first "app"
      map(proxyAddresses => {
        const appsWithoutKernel = proxyAddresses.filter(
          address => !this.isKernelAddress(address)
        );
      ...
      map(appProxies =>
        appProxies.filter(
          appProxy =>
            this.isApp(appProxy) || this.isKernelAddress(appProxy.proxyAddress)
        )
      )
    );
    ...
    const updatedApps$ = this.kernelProxy
      ...
      .pipe(
        ...
        filter(({ returnValues }) =>
          isKernelAppCodeNamespace(returnValues.namespace)
        ),

        // Merge with latest value of installedApps$ so we can return the full list of apps
        withLatestFrom(installedApps$, function updateApps(setAppEvent, apps) {
    ...
    this.apps = appsWithInfo$.pipe(publishReplay(1));
    this.apps.connect();
    ...
    const repo$ = apps$.pipe(
      ...
      filter(
        ([newRepoAppIds, updatedRepoAppIds]) =>
          ...
          mergeMap(async ({ appId, repoProxy }) => {
            ...
            return repoProxy.events("NewVersion").pipe(
              ...
              concatMap(async event => {
                ...
                const timestamp =
                  (await this.web3.eth.getBlock(event.blockNumber)).timestamp *
                  1000;

                const versionIndex = initialVersions.findIndex(
                  ({ versionId }) => versionId === eventVersionId
                );
                const versionInfo =
                  versionIndex === -1
              ...
              scan(
                ({ appId, versions }, newVersionInfo) => {
                  let newVersions = versions;
                  if (newVersionInfo) {
                    const versionIndex = versions.findIndex(
                      ({ versionId }) => versionId === newVersionInfo.versionId
                    );
      ...
      concatMap(async ({ repos, updatedRepoAppId: appId }) => {
        ...
        const currentVersion = Array.from(versions)
          ...
          .find(
            version =>
              version.contractAddress === installedRepoInfo.contractAddress
          );
    ...
    this.installedRepos = repo$.pipe(
      ...
      scan((repos, updatedRepo) => {
        const repoIndex = repos.findIndex(
          repo => repo.repoAddress === updatedRepo.repoAddress
        );
        if (repoIndex === -1) {
  ...
  initForwarders() {
    this.forwarders = this.apps.pipe(
      map(apps => apps.filter(app => app.isForwarder)),
      publishReplay(1)
    );
  ...
  initAppIdentifiers() {
    this.appIdentifiers = new BehaviorSubject({}).pipe(
      scan((identifiers, { address, identifier }) =>
  ...
  modifyAddressIdentity(address, metadata) {
    ...
    return Promise.reject(
      new Error(`Provider (${providerName}) not installed`)
    );
  }
   ...
   *
   * @param  {string} address Address to modify
   * @return {Promise} Reolved bgy the handler of identityIntents
   */
    ...
    if (this.identityProviderRegistrar.has(providerName)) {
      return new Promise((resolve, reject) => {
        this.identityIntents.next({
          ...
          reject(err) {
            reject(
              err || new Error("The identity modification was not completed")
            );
          }
        });
      });
  ...
  initNotifications() {
    ...
    this.notifications = new BehaviorSubject(cached).pipe(
      scan((notifications, { modifier, notification }) =>
    ...
    this.notifications.next({
      modifier: (notifications, notification) => {
        ...
        const newNotificationIndex = notifications.findIndex(
          notification =>
            new Date(notification.date).getTime() >= date.getTime()
        );
  ...
  async runApp(proxyAddress) {
    ...
    return sandboxMessengerProvider => {
      // Set up messenger
      const messenger = new Messenger(sandboxMessengerProvider);
      ...
      const handlerSubscription = handlers
        .combineRequestHandlers(
          ...
          handlers.createRequestHandler(
            ...
            handlers.externalPastEvents
          ),
          handlers.createRequestHandler(
            request$,
            "identify",
            handlers.appIdentifier
          ),
          handlers.createRequestHandler(
            request$,
            "address_identity",
            handlers.addressIdentity
          ),
          handlers.createRequestHandler(
            request$,
            "accounts",
      ...
      const shutdownAndClearCache = async () => {
        ...
        return Object.keys(await this.cache.getAll()).reduce(
          (promise, cacheKey) => {
            return promise.then(
              () =>
   ...
   */
  async getTransactionPathForIntentBasket(
    intentBasket,
    { checkMode = "all" } = {}
  ) {
    ...
    const intentPaths = await Promise.all(
      intentsToCheck.map(([destination, methodName, params]) =>
    ...
    if (pathsMatch) {
      ...
      const directTransactions = await Promise.all(
        intentBasket.map(async ([destination, methodName, params]) =>
          createDirectTransaction(
            sender,
            await this.getApp(destination),
            methodName,
            params,
            this.web3
          )
      ...
      if (intentPaths[0].length === 1) {
          ...
          const decoratedTransactions = await this.describeTransactionPath(
            await Promise.all(
              directTransactions.map(transaction =>
                this.applyTransactionGas(transaction)
              )
            )
          );
        ...
        const forwarderPath = intentPaths[0]
          ...
          .reduce(
            (path, nextForwarder) => {
              ...
              const encodedLastStep = encodeCallScript(
                Array.isArray(lastStep) ? lastStep : [lastStep]
              );
              return [
                createForwarderTransaction(nextForwarder, encodedLastStep),
                ...path
              ];
            },
        ...
        try {
          ...
          forwarderPath[0] = await this.applyTransactionGas(
            forwarderPath[0],
            true
          );
          return {
  ...
  async getACLTransactionPath(method, params) {
    ...
    if (functionArtifact.roles && functionArtifact.roles.length !== 0) {
      ...
      if (appIndex === -1 || roleIndex === -1) {
        throw new Error(
          `Method ${method} doesn't take _app and _role as input. Permission manager cannot be found.`
        );
      }

      const manager = await this.getPermissionManager(
        params[appIndex],
        params[roleIndex]
      );
    ...
    return Promise.all(
      path.map(async step => {
        ...
        if (Array.isArray(step)) {
          ...
          return (
            decoratedStep || Promise.all(step.map(this.describeTransactionPath))
          );
        }
        ...
        if (decoratedStep && decoratedStep.description) {
          try {
            const processed = await this.postprocessRadspecDescription(
  ...
  async applyTransactionGas(transaction, isForwarding = false) {
    ...
    if (transaction.pretransaction) {
      ...
      transaction.pretransaction = await this.applyTransactionGas(
    ...
    if (
      !transaction.gas ||
      (isForwarding && transaction.gas < recommendedGasLimit)
    ) {
   ...
   * @return {Promise<Array<Object>>} An array of Ethereum transactions that describe each step in the path
   */
  async calculateTransactionPath(
    sender,
    destination,
    methodName,
    params,
    ...
    const forwarders = await this.forwarders
      .pipe(first())
      .toPromise()
      .then(forwarders => forwarders.map(forwarder => forwarder.proxyAddress));
    ...
    if (finalForwarderProvided) {
      ...
      forwardersWithPermission = forwarders.filter(forwarder =>
        includesAddress(appsWithPermissionForMethod, forwarder)
      );
    }

    return this.calculateForwardingPath(
      sender,
      destination,
      directTransaction,
  ...
  async calculateForwardingPath(
    ...
    for (const forwarder of forwardersWithPermission) {
      ...
      if (await this.canForward(forwarder, sender, script)) {
        ...
        return [
          await this.applyTransactionGas(transaction, true),
          directTransaction
        ];
      }
    }
